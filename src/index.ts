...
//@ts-ignore typescript does not know request.recaptcha was set by the express-recaptcha middleware
  ...
  if (errors.isEmpty() === false) {
    ...
    return response.send(`<div class="alert alert-danger", role="alert"><strong>On Snap!</strong>${currentError.msg}`)
  }
  ...
  mailgunClient.messages.create(process.env.MAILGUN_DOMAIN, mailgun).then((msg: any) => response.send(
    `<div class='alert alert-danger' role='alert'><strong>Oh Snap3!</strong> Email Failed. Please Try Again.</div>`)).catch((error: any) => {

    console.error(errors)
    return response.send(`<div class='alert alert-danger' role='alert'><strong>Oh Snap3!</strong> There was a Recaptcha error. Please Try Again.</div>`)
  })
}
  ...
  app.listen(4200, () => {
    console.log('express successfullybuilt')
  })
